...
<script>
	function goUpdate() {
		...
		*/
		...
		board.method="post";
		board.action="Board?t_gubun=update"; // 첨부파일 form 사용시 GET 방식으로 구분 값을 직접 넘김 
		board.submit();
	}
</script>
		...
		<h1>BOARD</h1>
			...
			<table class="board-table" style="margin-bottom: 10px; width: 40%;">
				...
				<tr>
					...
					<td colspan="3">
						<input type="text" name="t_title" value="${board.getTitle()}" size="70" style="width: 90%"/>
					</td>
				</tr>
				<tr>
					<td class="th-td">내용</td>
					<td colspan="3">
						<textarea name="t_content" rows="30" cols="70" style="width: 90%; resize: none;">${board.getContent()}</textarea>
					</td>
				</tr>
				<tr>
					...
					<c:if test="${not empty board.getAttach()}">
						${board.getAttach()}
						&nbsp;&nbsp;&nbsp;
						삭제 <input type="checkbox" name="t_delete_checkbox" value="${board.getAttach()}"/>
						<!-- 기존 첨부파일명 담는 폼 -->
						<input type="hidden" name="t_original_attach" value="${board.getAttach()}"/>
					</c:if>
						<input type="file" name="t_attach" value="${board.getAttach()}"/>
					</td>
				</tr>
				<tr>
					<td class="th-td">글쓴이</td>
					<td>${board.getReg_name()}</td>
			...
			<section class="button">
				...
				&nbsp;&nbsp; / &nbsp;&nbsp;
				<a href="javascript:goUpdate()">UPDATE</a>
			</section>
		</section>
		</fieldset>
	</main>
